...
class Development implements ErrorHandlerInterface
{
    ...
    protected function displayDeveloperOutput(
        ...
        if ($exception) {
            $marker['exception_class'] = $this->linkClass(
                get_class($exception)
            );
            if ($code > 0) {
    ...
    protected function createBacktrace(array $traces)
    {
        ...
        foreach ($traces as $index => $step) {
            ...
            if (isset($step['file'])) {
                $backtrace .= $this->receiveCodeFragment(
                    $step['file'],
                    $step['line'],
                    3,
                    3
    ...
    protected function getBacktraceStepArguments($step)
    {
        ...
        foreach ($step['args'] as $argument) {
            $arguments .= strlen($arguments) === 0 ? '' : $this->tag(
                'span',
                ', ',
                ['class' => 'separator']
            );
     ...
     * @return string
     */
    protected function receiveCodeFragment(
        $filename,
        $lineNumber,
        $linesBefore = 3,
        $linesAfter = 3
